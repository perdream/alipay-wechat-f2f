...
function execute_precreate (ali_config,pri_config){
    return new Promise((resolve,reject)=>{
        ...
        axios({
            ...
            const rootName = config.alipay_f2f_conf.precreate_method.replace(/\./g, "_")+'_response';
            if(backJson.rootName != undefined){
                //返回数据的签名校验
                try{
                    if(!signHelper.verifyContent(backJson[rootName],backSign,config.alipay_f2f_conf.alipayPublicKey)){
